...
2. **Sub-Agent (MANDATORY ? platform-native):** Delegate the classification and task-update phase to **exactly one** `email-classifier` subagent. It semantic-matches EPD/course codes/contacts/geo/scope, updates tasks via `update_task.py`, registers ignores via `manage_ignore_candidates.py`, and returns a summary per `assistant_brain/formats/EMAIL_SYNC_FORMAT.md`.
...
3. **Verify & present:** every task section must contain BOTH `Emails:` and `Actions:` Ã¢â‚¬â€ add missing `Actions:` from the task's open asks (never surface completed ones). Then present the corrected summary.
