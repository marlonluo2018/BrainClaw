...
- This file is read by a Codex subagent; it is not an auto-discovered agent manifest.
- For each email sync, the root agent must spawn exactly one Codex subagent and instruct it to follow this entire file.
- The Codex subagent may process the sync and run only the task-update and ignore-registration commands authorized below.
-->
---
